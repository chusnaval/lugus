...
import lugus.model.ImdbTitlePrincipalsId;

public interface ImdbTitlePrincipalsRepository extends JpaRepository<ImdbTitlePrincipals, ImdbTitlePrincipalsId> {

	@Query("SELECT i FROM ImdbTitlePrincipals i WHERE i.id.nconst = :nConst and i.id.category != 'self' and i.id.category != 'archive_footage'")
